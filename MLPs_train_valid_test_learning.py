...
def mlp_train(model, train_loader, loss_func, optimizer):
    """
    Train the MLP model for one epoch.
...
def mlp_valid(model, valid_loader, loss_func):
    """
    Validate the MLP model on the validation dataset.

    Parameters:
    - model: The MLP model to be validated.
...
def mlp_test(model, test_loader, loss_func):
    """
    Test the MLP model on the test dataset and calculate additional classification metrics.

    Parameters:
    - model: The MLP model to be tested.
...
def mlp_learning(model, train_loader, valid_loader, loss_func, optimizer, save_directory, model_name, max_epoch=500, patience=20):
    """
    ...
    for epoch in range(max_epoch):
        train_loss, train_acc = mlp_train(model, train_loader, loss_func, optimizer)
        val_loss, val_acc = mlp_valid(model, valid_loader, loss_func)
